...
namespace Pale::ply_detail {
    ...
    inline void canonicalizeFrameFaceUp(glm::vec3 &tangentU,
                                        glm::vec3 &tangentV)
    {
        ...
        if (glm::dot(tvOrtho, tvOrtho) < epsilon) {
            tvOrtho = (glm::abs(tangentU.y) < 0.9f)
                      ? glm::vec3(0, 1, 0)
                      : glm::vec3(1, 0, 0);
            tvOrtho -= glm::dot(tvOrtho, tangentU) * tangentU;
        }
...
export namespace Pale {
    ...
    struct PLYPointLoader : IAssetLoader<PointAsset> {
        AssetPtr<PointAsset> load(const AssetHandle & /*id*/, const AssetMeta &meta) override {
            ...
            const bool looksCustom = ply_detail::hasAll(vertexProps,
                                                        {
            ...
            const bool looks2DGS = ply_detail::hasAll(vertexProps,
                                                      {
                                                          ...
                                                          "scale_0", "scale_1", "rot_0", "rot_1", "rot_2", "rot_3"
                                                      });

            if (!looksCustom && !looks2DGS) {
            ...
            if (looksCustom) {
                ...
                opacityData = plyFile.request_properties_from_element("vertex", {"opacity"});
                scale2D = plyFile.request_properties_from_element("vertex", {"scale_0", "scale_1"});
            ...
            if (looksCustom) {
                ...
                for (std::size_t i = 0; i < vertexCount; ++i) {
                    ...
                    ply_detail::canonicalizeFrameFaceUp(tangentU, tangentV);
            ...
            // -------- 3DGS → 2D surface-aligned representation --------
            {
                ...
                Log::PA_INFO(
                    "PLYPointLoader: loaded {} 3DGS splats (DC color, rot[0:1] → in-plane basis, scale[0:1] → radii)",
                    vertexCount);
                return pointAsset;
            }
        }
