...
namespace Pale {
    ...
    inline float3 gatherDiffuseIrradianceAtPoint(
        ...
        const float slabHalfThickness = sycl::fmax(1e-5f, 0.001f * radius);

        // 2D Epanechnikov-style normalization over the tangent disk.
        //const float kernelNormalization = 2.0f / (M_PIf * radiusSquared);

        // Uniform 2D disk normalization over the tangent plane.
        const float kernelNormalization = 1.0f / (M_PIf * radiusSquared);
        ...
        const sycl::int3 minCell = worldToCellClamped(queryPositionWorld - supportOffset, grid);
        const sycl::int3 maxCell = worldToCellClamped(queryPositionWorld + supportOffset, grid);
        ...
        for (int cellZ = minCell.z(); cellZ <= maxCell.z(); ++cellZ) {
            for (int cellY = minCell.y(); cellY <= maxCell.y(); ++cellY) {
                for (int cellX = minCell.x(); cellX <= maxCell.x(); ++cellX) {
                    const uint32_t cellId = linearCellIndex(sycl::int3{cellX, cellY, cellZ}, grid.gridResolution);
                    ...
                    for (uint32_t sortedIndex = start; sortedIndex < end; ++sortedIndex) {
                        ...
                        if (dot(normalizedQueryNormal, -photon.incomingDirection) <= 0.0f) {
                            continue;
                        }

                        const float3 offsetWorld = photon.position - queryPositionWorld;

                        const float planeDistance = dot(offsetWorld, normalizedQueryNormal);
                        ...
                        const float3 tangentialOffset = offsetWorld - planeDistance * normalizedQueryNormal;
                        const float tangentialDistanceSquared = dot(tangentialOffset, tangentialOffset);

                        if (tangentialDistanceSquared > radiusSquared) {
                            continue;
                        }

                        irradiance += photon.flux * kernelNormalization;
                    }
                }
            }
        }
