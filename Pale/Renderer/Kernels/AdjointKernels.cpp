...
namespace Pale {
    ...
    void launchRayGenAdjointKernel(RenderPackage &pkg, int spp, uint32_t cameraIndex) {
        ...
        queue
                .submit([&](sycl::handler &commandGroupHandler) {
                    ...
                    commandGroupHandler.parallel_for<struct RayGenAdjointKernelTag>(
                        sycl::range<1>(raysPerSet), [=](sycl::id<1> globalId) {
                            ...
                            const uint64_t seed = rng::makeSeed(renderSeed, globalRayIndex, spp, rng::kStreamRayGen,
                                                                spp * spp);
                            ...
                            Ray primaryRay = makePrimaryRayFromPixelJitteredFov(
                                sensor.camera, static_cast<float>(pixelX), static_cast<float>(pixelY), jitterX,
                                jitterY);
                            ...
                            for (uint32_t inlineTraversalIndex = 0u; inlineTraversalIndex < kMaxSplatEventsPerRay; ++
                                ...
                                if (instance.geometryType == GeometryType::PointCloud) {
                                    ...
                                    const PointCloudLocalLayer localLayer = collectPointCloudLocalLayer(
                                        currentRayState.ray, worldHit, instance, scene, localLayerDepthEpsilon,
                                        maxLocalSurfelHits, localLayerNormalCosineThreshold);
                                    ...
                                    for (uint32_t localHitIndex = 0u; localHitIndex < localLayer.hitCount; ++
                                         localHitIndex) {
                                        ...
                                        sampleUniformHemisphereAroundNormal(
                                            rng, orientedNormal, sampledOutgoingDirectionWorld, uniformHemispherePdf);
                                        ...
                                        if (hasPendingState) {
                                            ...
                                            if (previousAdjointStage.valid) {
                                                const Point &storedSurfel = scene.points[previousAdjointStage.current.
                                                ...
                                                segmentGeometryFromStoredVertex = computeGeometricTermValue(
                                                    storedState.position, liveState.position,
                                                    storedState.orientedNormal, liveState.orientedNormal);
                                            ...
                                            if (previousCameraSegment.valid) {
                                                ...
                                                measurementEvent.directLightEps[slabRecordCount] = localLayer.
                                                        directLightEpsilon[localHitIndex];
                                                ++slabRecordCount;
                                                /*
                                    ...
                                    if (slabRecordCount > 0u) {
                                        ...
                                        appendEventAtomic(intermediates.countMeasurementEvents,
                                        ...
                                        appendAdjointMeasurementDirectPointLightSlabEvents(
                                            settings, scene, intermediates, measurementEvent, currentRayState,
                                            qReflect);
                                    }
        ...
        queue
                .submit([&](sycl::handler &cgh) {
                    cgh.parallel_for<>(sycl::range<1>(measurementEventCount), [=](sycl::id<1> globalId) {
                        ...
                        const float3 pathWeightAtTarget =
                            eventRecord.xPathThroughput * eventRecord.transmission;                        float alphaEff[kMaxLocalSurfelHits];
                        float3 slabDirectRadiance[kMaxLocalSurfelHits], targetSlabRadiance{0.0f}, targetAnchorPosition{
                            0.0f
                        };
                        ...
                        for (uint32_t traversalIndex = 0u; traversalIndex < maxSplatEventsPerRay; ++traversalIndex) {
                            ...
                            for (uint32_t hitIndex = 0u; hitIndex < occludingLayer.hitCount; ++hitIndex) {
                                ...
                                if (localHit.primitiveIndex == kInvalidIndex || localHit.primitiveIndex >= pointCount)
                                    continue;
                                ...
                                if (oneMinusR2 > 1.0e-8f) {
                                    ...
                                    if (sycl::fabs(nDotD) > 1.0e-8f && scaleU > 1.0e-12f && scaleV > 1.0e-12f) {
                                        ...
                                        gradPosition =
                                                occluderSurfel.opacity * (dAlphaGeomDu * dUdP + dAlphaGeomDv * dVdP);
                        ...
                        for (uint32_t parameterIndex = 0u; parameterIndex < slabCount; ++parameterIndex) {
                            ...
                            if (appendGradientRecordBounded(gradientRecordCounter, gradientRecords,
                                                            gradientRecordCapacity, gradientRecord)) {
                        ...
                        for (uint32_t reverseIndex = storedOccluderCount; reverseIndex > 0u; --reverseIndex) {
                            ...
                            const float scale = -occluder.prefixTransmittance * suffixTransmittance *
                                                scalarWeightOcclusion * invSpp;
    ...
    static void measurementGradientEventXY(RenderPackage &pkg, uint32_t eventCount, uint32_t cameraIndex) {
        ...
        queue
                .submit([&](sycl::handler &cgh) {
                    cgh.parallel_for<class firstHitGradientEventTag>(sycl::range<1>(eventCount),
                                                                     [=](sycl::id<1> globalId) {
                                                                         const uint32_t eventIndex = static_cast<
                                                                             uint32_t>(globalId[0]);
                                                                         const MeasurementGradientEventXY eventRecord =
                                                                                 measurementEvents[eventIndex];
                                                                         const uint32_t slabCount = eventRecord.
                                                                                 surfelSlabCount;
                                                                         if (slabCount == 0u || slabCount >
                                                                             kMaxLocalSurfelHits) return;
                                                                         const float3 lightPositionW = eventRecord.
                                                                                 pointLightPositionW;
                                                                         const float3 pointLightIntensity = eventRecord.
                                                                                 pointLightRadiantIntensity;
                                                                         const float3 pathWeight = eventRecord.
                                                                                 xPathThroughput;
                                                                         for (uint32_t localIndex = 0u;
                                                                              localIndex < slabCount; ++localIndex) {
                                                                             const PointCloudSurfaceRecord &xSurface =
                                                                                     eventRecord.xSurface[localIndex];
                                                                             const uint32_t xPrimitiveIndex = xSurface.
                                                                                     primitiveIndex;
                                                                             if (xPrimitiveIndex == kInvalidIndex ||
                                                                                 xPrimitiveIndex >= pointCount) continue
                                                                                     ;
                                                                             const float layerWeight = eventRecord.
                                                                                     layerWeights[localIndex];
                                                                             if (layerWeight <= 0.0f) continue;
                                                                             const Point &surfelX = scene.points[
                                                                                 xPrimitiveIndex];
                                                                             const ReconstructedSurfelState xState =
                                                                                     reconstructSurfelState(
                                                                                         surfelX, xSurface);
                                                                             PointLightGeometry lightGeometry{};
                                                                             if (!computePointLightGeometry(
                                                                                 xState.position, xState.orientedNormal,
                                                                                 lightPositionW,
                                                                                 lightGeometry)) continue;
                                                                             const float3 brdfX =
                                                                                     surfelX.alpha_r * surfelX.albedo *
                                                                                     M_1_PIf;
                                                                             const float3
                                                                                     transportWithoutTauAndGeometric =
                                                                                             pointLightIntensity *
                                                                                             layerWeight * brdfX;
                                                                             const float
                                                                             ...
                                                                             const float targetDistanceSquared = dot(
                                                                                 segmentVector, segmentVector);
                                                                             if (targetDistanceSquared <= 1.0e-12f)
                                                                                 continue;
                                                                             const float targetDistance = sycl::sqrt(
                                                                             ...
                                                                             ray.origin =
                                                                                     xState.position + rayDirection *
                                                                                     sycl::fmax(
                                                                                         RayEpsilon,
                                                                                         eventRecord.directLightEps[
                                                                                             localIndex]);
                                                                             OccluderDerivative occluderDerivatives[
                                                                                 kMaxShadowOccluderRecords];
                                                                             uint32_t storedOccluderCount = 0u;
                                                                             float segmentTransmittance = 1.0f;
                                                                             for (uint32_t traversalIndex = 0u;
                                                                                 ...
                                                                                 const float3 hitVector =
                                                                                         shadowHit.hitPositionW - xState
                                                                                         .position;
                                                                                 const float hitDistance = sycl::sqrt(
                                                                                     dot(hitVector, hitVector));
                                                                                 if (hitDistance >= targetDistance -
                                                                                     RayEpsilon) break;
                                                                                 buildIntersectionNormal(
                                                                                     scene, shadowHit);
                                                                                 const InstanceRecord &instance = scene.
                                                                                         instances[shadowHit.
                                                                                             instanceIndex];
                                                                                 if (instance.geometryType !=
                                                                                     GeometryType::PointCloud) break;
                                                                                 const PointCloudLocalLayer
                                                                                         occludingLayer =
                                                                                                 collectPointCloudLocalLayer(
                                                                                                     ...
                                                                                                     localLayerNormalCosineThreshold);
                                                                                 float prefixWithinLayer = 1.0f;
                                                                                 for (uint32_t hitIndex = 0u;
                                                                                     hitIndex < occludingLayer.hitCount;
                                                                                     ++hitIndex) {
                                                                                     const LocalSurfelLayerHit &localHit
                                                                                             = occludingLayer.hits[
                                                                                                 hitIndex];
                                                                                     const uint32_t
                                                                                             occluderPrimitiveIndex =
                                                                                                     localHit.
                                                                                                     primitiveIndex;
                                                                                     if (occluderPrimitiveIndex ==
                                                                                         kInvalidIndex ||
                                                                                         occluderPrimitiveIndex >=
                                                                                         pointCount) continue;
                                                                                     if (isPrimitiveInMeasurementSlab(
                                                                                             eventRecord,
                                                                                             occluderPrimitiveIndex) ||
                                                                                         occluderPrimitiveIndex ==
                                                                                         eventRecord.
                                                                                         pointLightPrimitiveIndex)
                                                                                         continue;
                                                                                     const Point &occluderSurfel = scene
                                                                                             .points[
                                                                                                 occluderPrimitiveIndex];
                                                                                     float3 occluderNormal = normalize(
                                                                                         cross(occluderSurfel.tanU,
                                                                                             occluderSurfel.tanV));
                                                                                     if (dot(occluderNormal,
                                                                                         -rayDirection) < 0.0f)
                                                                                         occluderNormal = -
                                                                                                 occluderNormal;
                                                                                     const float alphaGeom = localHit.
                                                                                             alphaGeom;
                                                                                     const float alphaEffective =
                                                                                             sycl::clamp(
                                                                                                 occluderSurfel.opacity
                                                                                     ...
                                                                                     const float oneMinusAlpha =
                                                                                             sycl::fmax(
                                                                                                 0.0f,
                                                                                                 1.0f - alphaEffective);
                                                                                     const float2 uv = phiInverse(
                                                                                         localHit.hitPositionW,
                                                                                         occluderSurfel);
                                                                                     const float u = uv.x(), v = uv.y();
                                                                                     const float oneMinusR2 =
                                                                                             1.0f - u * u - v * v;
                                                                                     float3 gradPosition{0.0f},
                                                                                             gradWorldRotation{0.0f};
                                                                                     float gradScaleU = 0.0f, gradScaleV
                                                                                             = 0.0f, gradBeta = 0.0f;
                                                                                     if (oneMinusR2 > 1.0e-8f) {
                                                                                         const float scaleU =
                                                                                                 occluderSurfel.scale.
                                                                                                 x();
                                                                                         const float scaleV =
                                                                                                 occluderSurfel.scale.
                                                                                                 y();
                                                                                         ...
                                                                                         const float dAlphaGeomDu =
                                                                                                 -2.0f * betaScale * u *
                                                                                                 alphaGeom / oneMinusR2;
                                                                                         const float dAlphaGeomDv =
                                                                                                 -2.0f * betaScale * v *
                                                                                                 alphaGeom / oneMinusR2;
                                                                                         const float nDotD = dot(
                                                                                             occluderNormal,
                                                                                             rayDirection);
                                                                                     ...
                                                                                     if (storedOccluderCount <
                                                                                         kMaxShadowOccluderRecords) {
                                                                                         OccluderDerivative &record =
                                                                                                 occluderDerivatives[
                                                                                                     storedOccluderCount
                                                                                                     ++];
                                                                                         record = OccluderDerivative{};
                                                                                         ...
                                                                                         record.gradEta = alphaGeom;
                                                                                         record.gradBeta = gradBeta;
                                                                                         record.gradAlphaWrtSegmentStart
                                                                                                 = computeAlphaEffectiveGradientWrtSegmentStart(
                                                                                                     occluderSurfel,
                                                                                                     localHit,
                                                                                                     xState.position,
                                                                                                     lightPositionW);
                                                                                         record.prefixTransmittance =
                                                                                                 segmentTransmittance *
                                                                                                 prefixWithinLayer;
                                                                                         record.oneMinusAlpha =
                                                                                                 oneMinusAlpha;
                                                                                     }
                                                                                     prefixWithinLayer *= oneMinusAlpha;
                                                                                 }
                                                                                 ...
                                                                                 ray.origin += ray.direction * (
                                                                                     occludingLayer.furthestT +
                                                                                     RayEpsilon);
                                                                             }
                                                                             float suffixTransmittance = 1.0f;
                                                                             float3 gradTauWrtSegmentStart{0.0f};
                                                                             for (uint32_t reverseIndex =
                                                                                         storedOccluderCount;
                                                                                 reverseIndex > 0u; --reverseIndex) {
                                                                                 const OccluderDerivative &occluder =
                                                                                         occluderDerivatives[
                                                                                             reverseIndex - 1u];
                                                                                 const float dTauDAlpha =
                                                                                         -occluder.prefixTransmittance *
                                                                                         suffixTransmittance;
                                                                                 gradTauWrtSegmentStart += dTauDAlpha *
                                                                                         occluder.
                                                                                         gradAlphaWrtSegmentStart;
                                                                                 const float visibilityScale =
                                                                                         dTauDAlpha * lightGeometry.
                                                                                         geometricTerm *
                                                                                         scalarWeightWithoutTauAndGeometric
                                                                                         * invSpp;
                                                                                 SurfelGradientRecord gradientRecord =
                                                                                         makeZeroSurfelGradientRecord(
                                                                                             occluder.primitiveIndex);
                                                                                 const float3 position =
                                                                                         visibilityScale * occluder.
                                                                                         gradPosition, rotation =
                                                                                         visibilityScale * occluder.
                                                                                         gradRotation;
                                                                                 gradientRecord.gradPositionX = position
                                                                                         .x();
                                                                                 gradientRecord.gradPositionY = position
                                                                                         .y();
                                                                                 gradientRecord.gradPositionZ = position
                                                                                         .z();
                                                                                 ...
                                                                                 gradientRecord.gradScaleV =
                                                                                         visibilityScale * occluder.
                                                                                         gradScaleV;
                                                                                 gradientRecord.gradRotationX = rotation
                                                                                         .x();
                                                                                 gradientRecord.gradRotationY = rotation
                                                                                         .y();
                                                                                 gradientRecord.gradRotationZ = rotation
                                                                                         .z();
                                                                                 ...
                                                                                 if (appendGradientRecordBounded(
                                                                                     ...
                                                                                     accumulateDebugGradientIfSelected(
                                                                                         ...
                                                                                         settings.
                                                                                         surfelIndexForDebugImages,
                                                                                         xSurface.pathId,
                                                                                         gradientRecord);
                                                                                 }
                                                                                 suffixTransmittance *= occluder.
                                                                                         oneMinusAlpha;
                                                                             }
                                                                             const float3 gradEdgeWrtHitPosition =
                                                                                     ...
                                                                                     * (segmentTransmittance *
                                                                                         ...
                                                                                         lightGeometry.geometricTerm *
                                                                                         gradTauWrtSegmentStart) *
                                                                                     invSpp;
                                                                             const float3x3 hitPositionJacobian =
                                                                                     planeHitPointIntersectionJacobian(
                                                                                         xSurface.incomingDirection,
                                                                                         xState.orientedNormal);
                                                                             const float3 gradPosition =
                                                                                     transpose(hitPositionJacobian) *
                                                                                     gradEdgeWrtHitPosition;
                                                                             const float3x3 hitRotationJacobian =
                                                                                     planeHitPointRotationJacobian(
                                                                                         sensor.camera.pos,
                                                                                         xSurface.incomingDirection,
                                                                                         surfelX.position,
                                                                                         xState.orientedNormal);
                                                                             const float3 normalRotationContribution =
                                                                                     scalarWeightWithoutTauAndGeometric
                                                                                     * segmentTransmittance * cross(
                                                                                         xState.orientedNormal,
                                                                                         lightGeometry.
                                                                                         gradientWrtSurfaceNormal) *
                                                                                     invSpp;
                                                                             const float3 gradWorldRotation =
                                                                                     transpose(hitRotationJacobian) *
                                                                                     gradEdgeWrtHitPosition +
                                                                                     normalRotationContribution;
                                                                             const float3 gradRotation =
                                                                                     computeLocalRotationGradientFromWorldRotationGradient(
                                                                                         surfelX.tanU, surfelX.tanV,
                                                                                         gradWorldRotation);
                                                                             SurfelGradientRecord targetRecord =
                                                                                     makeZeroSurfelGradientRecord(
                                                                                         xPrimitiveIndex);
                                                                             targetRecord.gradPositionX = gradPosition.
                                                                                     x();
                                                                             targetRecord.gradPositionY = gradPosition.
                                                                                     y();
                                                                             targetRecord.gradPositionZ = gradPosition.
                                                                                     z();
                                                                             targetRecord.gradRotationX = gradRotation.
                                                                                     x();
                                                                             targetRecord.gradRotationY = gradRotation.
                                                                                     y();
                                                                             targetRecord.gradRotationZ = gradRotation.
                                                                                     z();
                                                                             if (appendGradientRecordBounded(
                                                                                 gradientRecordCounter, gradientRecords,
                                                                                 gradientRecordCapacity,
                                                                                 targetRecord)) {
                                                                                 accumulateDebugGradientIfSelected(
                                                                                     debugImage,
                    ...
                    commandGroupHandler.parallel_for<struct reduceSurfelGradientRecords>(
                        sycl::range<1>(gradientRecordCount), [=](sycl::id<1> globalId) {
                            ...
                            const bool validGradientRecord =
                                    ...
                                    isValidGradientComponent(gradientRecord.gradAlbedoG) && isValidGradientComponent(
                            ...
                            atomicAddUint32(
                                gradients.gradPositionRecordCountPerPrimitivePerCamera[primitiveCameraIndex], 1u);
                            if (gradientRecord.hasCloneSignal != 0u) {
                                ...
                                atomicAddUint32(
                                    gradients.cloneSignalRecordCountPerPrimitivePerCamera[primitiveCameraIndex], 1u);
                            }
                    ...
                    commandGroupHandler.parallel_for<class ComputePerPrimitiveTranslationGradientStatsKernel>(
                        sycl::range<1>(pointCount), [=](sycl::id<1> globalId) {
                            ...
                            for (uint32_t cameraIndex = 0u; cameraIndex < cameraSlotCount; ++cameraIndex) {
                                ...
                                if (gradients.gradPositionRecordCountPerPrimitivePerCamera[primitiveCameraIndex] ==
                                    0u) { continue; }
                                const float3 cameraGradient = gradients.gradPositionPerPrimitivePerCamera[
                                    primitiveCameraIndex];
                                const float cameraGradientSquaredNorm =
                                        cameraGradient.x() * cameraGradient.x() + cameraGradient.y() * cameraGradient.
                                        y() + cameraGradient.z() * cameraGradient.z();
                            ...
                            const float meanGradientSquaredNorm =
                                    meanGradient.x() * meanGradient.x() + meanGradient.y() * meanGradient.y() +
                            ...
                            for (uint32_t cameraIndex = 0u; cameraIndex < cameraSlotCount; ++cameraIndex) {
                                const uint32_t primitiveCameraIndex = primitiveIndex * cameraSlotCount + cameraIndex;
                                if (gradients.cloneSignalRecordCountPerPrimitivePerCamera[primitiveCameraIndex] == 0u) {
                                    continue;
                                }
                                const float3 cameraCloneSignal = gradients.cloneSignalPerPrimitivePerCamera[
                                    primitiveCameraIndex];
                                const float cameraCloneSignalSquaredNorm =
                                        cameraCloneSignal.x() * cameraCloneSignal.x() + cameraCloneSignal.y() *
                                        cameraCloneSignal.y() + cameraCloneSignal.z() * cameraCloneSignal.z();
                            ...
                            const float meanCloneSignalSquaredNorm =
                                    meanCloneSignal.x() * meanCloneSignal.x() + meanCloneSignal.y() * meanCloneSignal.
                                    y() + meanCloneSignal.z() * meanCloneSignal.z();
                            const float meanCloneSignalNorm = sycl::sqrt(meanCloneSignalSquaredNorm);
                            const float meanPerCameraCloneSignalNorm =
                                    cloneSignalNormSum * inverseCloneSignalActiveCameraCount;
                            ...
                            const float cloneSignalCoherence =
                                    meanCloneSignalNorm / (meanPerCameraCloneSignalNorm + epsilon);
                            const float clampedCloneSignalCoherence = sycl::fmin(
                                1.0f, sycl::fmax(0.0f, cloneSignalCoherence));
    ...
    static inline float3 loadFloat4Rgb(const float4 &v) { return float3{v.x(), v.y(), v.z()}; }
                        ...
                        sycl::range<1>(pixelCount), [=](sycl::id<1> tid) {
                            ...
                            auto rayDirForPixel = [&](uint32_t px, uint32_t py) -> float3 {
                                Ray ray = makePrimaryRayFromPixelJitteredFov(
                                    sensor.camera, static_cast<float>(px), static_cast<float>(py), 0.0f, 0.0f);
                            ...
                            if (sycl::fabs(denomL) <= 1e-8f || sycl::fabs(denomR) <= 1e-8f || sycl::fabs(denomU) <=
                                1e-8f || sycl::fabs(denomD) <= 1e-8f) { return; }
                ...
                .submit([&](sycl::handler &cgh) {
                    cgh.parallel_for<class SurfaceRegularizersBackwardKernel>(
                        sycl::range<1>(pixelCount), [=](sycl::id<1> tid) {
                            ...
                            const bool useDepthDistortion = enableDepthDistortionRegularizer && sycl::fabs(
                                                                depthDistortionAdjoint) > 1.0e-12f;
                            ...
                            for (uint32_t traversalIndex = 0u;
                                ...
                                if (primitiveIndex == kInvalidIndex || primitiveIndex >= pointCount) {
                                    regularizerRay.origin =
                                            worldHit.hitPositionW + regularizerRay.direction * RayEpsilon;
                                    continue;
                                }
                                const Point &surfel = scene.points[primitiveIndex];
                                ...
                                const float depth = dot(worldHit.hitPositionW - sensor.camera.pos,
                            ...
                            const float selectedMeanDepth = accumulatedWeight > kAlphaEpsilon ? accumulatedWeightedDepth / accumulatedWeight : 0.0f;
                            // =============================================================
                            // PASS 2:
                            //
                            ...
                            for (int32_t reverseIndex = static_cast<int32_t>(hitCount) - 1; reverseIndex >= 0; --
                                 reverseIndex) {
                                ...
                                if (useDepthDistortion) {
                                    ...
                                    if (!kDetachDepthDistortionWeights) {
                                        barWeightDepth =
                                        (hit.ndcDepth * hit.ndcDepth * accumulatedWeight +
                                ...
                                if (useNormalConsistency) {
                                    ...
                                    if (useSelectedDepth && normalFromDepthUseMeanDepth && accumulatedWeight > kAlphaEpsilon) {
                                        ...
                                        barWeightNormal += (hit.depth - selectedMeanDepth) * invAccumulatedWeight * selectedDepthAdjoint;
                                    }
                                    if (useSelectedDepth && !normalFromDepthUseMeanDepth && hitIndex == medianHitIndex) { barDepthNormal += selectedDepthAdjoint; }
                                }
                                // =========================================================
                                ...
                                if (useDepthDistortion && !kDetachDepthDistortionWeights) {
                                    barAlphaDepth =
                                            hit.transmittanceBefore * (barWeightDepth - barNextTransmittanceDepth);
                                ...
                                if (useNormalConsistency && !kDetachNormalConsistencyWeights) {
                                    barAlphaNormal =
                                            hit.transmittanceBefore * (barWeightNormal - barNextTransmittanceNormal);
                                ...
                                const SurfaceRegularizerConstituentGradient depthGradient =
                                        differentiateSurfaceRegularizerConstituent(
                                            surfel, hit, rayDirection, sensor.camera.forward, barAlphaDepth,
                                            barDepthDepth, float3{0.0f});
                                const SurfaceRegularizerConstituentGradient normalGradient =
                                        differentiateSurfaceRegularizerConstituent(
                                            surfel, hit, rayDirection, sensor.camera.forward, barAlphaNormal,
                                            barDepthNormal, barOrientedNormal);
                                // =========================================================
                                ...
                                if (useDepthDistortion) {
                                    atomicAddFloat3(depthGradients.gradPosition[hit.primitiveIndex],
                                                    depthGradient.position);
                                    ...
                                    atomicAddFloat3(depthGradients.gradRotation[hit.primitiveIndex],
                                                    depthRotationGradient);
                                    if (!kDetachDepthDistortionWeights) {
                                        atomicAddFloat2(depthGradients.gradScale[hit.primitiveIndex], float2{
                                                            depthGradient.scaleU, depthGradient.scaleV
                                                        });
                                        atomicAddFloat(depthGradients.gradOpacity[hit.primitiveIndex],
                                                       depthGradient.opacity);
                                ...
                                if (useNormalConsistency) {
                                    atomicAddFloat3(normalGradients.gradPosition[hit.primitiveIndex],
                                                    normalGradient.position);
                                    ...
                                    atomicAddFloat3(normalGradients.gradRotation[hit.primitiveIndex],
                                                    normalRotationGradient);
                                    atomicAddFloat2(normalGradients.gradScale[hit.primitiveIndex], float2{
                                                        normalGradient.scaleU, normalGradient.scaleV
                                                    });
                                    atomicAddFloat(normalGradients.gradOpacity[hit.primitiveIndex],
                                                   normalGradient.opacity);
                                ...
                                // =========================================================
                                if (writeDebugImages) {
                                    const float appliedDepthScaleU = kDetachDepthDistortionWeights
                                                                         ? 0.0f
                                                                         : depthGradient.scaleU;
                                    const float appliedDepthScaleV = kDetachDepthDistortionWeights
                                                                         ? 0.0f
                                                                         : depthGradient.scaleV;
                                    const float appliedDepthOpacity = kDetachDepthDistortionWeights
                                                                          ? 0.0f
                                                                          : depthGradient.opacity;
                                    const float appliedDepthBeta = kDetachDepthDistortionWeights
                                                                       ? 0.0f
                                                                       : depthGradient.beta;
                                    ...
                                    accumulateDebugGradientIfSelected(debugImage, settings.renderDebugGradientImages,
                                                                      settings.surfelIndexForDebugImages, pixelIndex,
                                                                      debugRecord);
                                }
                            }
                        });
    ...
    void adjointContributionKernels(RenderPackage &pkg, uint32_t measurementEventCount,
                                    uint32_t measurementTwoPointEventCount, uint32_t materialVertexEventCount,
                                    uint32_t materialEndEdgeEventCount, uint32_t materialStartEdgeEventCount,
