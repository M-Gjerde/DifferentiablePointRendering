                                                  ...
                                                  rng::Xorshift128& rng128)
{
    ...
    auto flushCurrentGroup = [&](rng::Xorshift128& rng)->bool {
        ...
        for (size_t i = 0; i < groupAlphas.size(); ++i) {
            ...
            if (rng.nextFloat() < probabilityFirstHere) {
                ...
                localHitOut.transmissivity = cumulativeTransmittanceBefore; // transmittance before the accepted event
    ...
    while (!traversalStack.empty()) {
        ...
        if (node.triCount == 0) {
            ...
            const bool hitLeft  = computeAabbEntry(rayObject, bvhNodes[leftIndex],  inverseDirection, bestAcceptedTHit, leftTEntry);
            const bool hitRight = computeAabbEntry(rayObject, bvhNodes[rightIndex], inverseDirection, bestAcceptedTHit, rightTEntry);
        ...
        BoundedVector<float, 64> leafTHits;
        BoundedVector<float, 64> leafAlphas;
        BoundedVector<uint32_t, 64> leafIndices;
        leafTHits.clear(); leafAlphas.clear(); leafIndices.clear();
        ...
        BoundedVector<int,   256> order;
        BoundedVector<float, 256> tempKeys;
        order.clear(); tempKeys.clear();
        for (int i = 0; i < leafCount; ++i) { order.pushBack(i); tempKeys.pushBack(leafTHits[i]); }
        ...
        for (int k = 0; k < leafCount; ++k) {
            ...
            if (groupTHits.size() == 0 || sycl::fabs(tHit - currentGroupDepth) <= sameDepthEpsilon) {
                ...
                if (flushCurrentGroup(rng128)) { foundAcceptedScatter = true; bestAcceptedTHit = localHitOut.t; break; }
                currentGroupDepth = tHit;
                groupTHits.clear(); groupAlphas.clear(); groupIndices.clear();
                (void)groupTHits.pushBack(tHit);
                (void)groupAlphas.pushBack(alpha);
                (void)groupIndices.pushBack(surfelIndex);
            }
    ...
    if (!foundAcceptedScatter && groupTHits.size() > 0) {
        if (flushCurrentGroup(rng128)) { foundAcceptedScatter = true; bestAcceptedTHit = localHitOut.t; }
    }
                                         ...
                                         rng::Xorshift128& rng128)
{
    ...
    while (!traversalStack.empty()) {
        ...
        if (node.count == 0) {
            ...
            const bool hitLeft  = computeAabbEntry(rayWorld, tlasNodes[leftIndex],  inverseDirectionWorld, bestWorldTHit, leftTEntry);
            const bool hitRight = computeAabbEntry(rayWorld, tlasNodes[rightIndex], inverseDirectionWorld, bestWorldTHit, rightTEntry);
        ...
        if (acceptedHitInInstance) {
            ...
            const float3 hitPointWorld = toWorldPoint(rayObject.origin + localHit.t * rayObject.direction, objectToWorld);
            const float tWorld = dot(hitPointWorld - rayWorld.origin, rayWorld.direction); // assumes normalized direction
            ...
            worldHitOut->transmissivity = transmittanceProduct * (instance.geometryType == GeometryType::PointCloud ? localHit.transmissivity : 1.0f);
        ...
        if (!acceptedHitInInstance && instance.geometryType == GeometryType::PointCloud && localHit.hasVisibilityTest) {
