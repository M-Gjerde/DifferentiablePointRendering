...
DEFAULT_DATASETS: tuple[DatasetConfig, ...] = (
    DatasetConfig("dragon_10", "rgs_dragon_10", "dragon.ply"),
    DatasetConfig("teapot_10", "rgs_teapot_10", "teapot.ply"),
    DatasetConfig("horse_10", "rgs_horse_10", "horse.ply"),
    DatasetConfig("lego_10", "rgs_lego_10", "lego.ply"),
    DatasetConfig("plant_10", "rgs_plant_10", "plant.ply"),
)
...
def resolve_reconstruction_path(output_root: Path, dataset: DatasetConfig, iteration: int | None) -> Path:
    ...
    if iteration is not None:
        ...
        return require_existing_path(
            reconstruction_path,
            f"RadiosityGS reconstruction for dataset '{dataset.name}' at iteration {iteration}",
        )

    train_root = require_existing_path(dataset_root / "train", f"RadiosityGS train folder for dataset '{dataset.name}'")
    ...
    if not reconstructions:
        raise FileNotFoundError(
            f"Could not find latest RadiosityGS reconstruction for dataset '{dataset.name}'. "
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Chamfer evaluation over multiple RadiosityGS datasets.")

    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/home/magnus/projects/RadiosityGS/output"),
        help="Root folder containing RadiosityGS output folders.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30_000,
        help="RadiosityGS iteration folder to evaluate, e.g. 7000 or 30000. Use -1 to select latest ours_*/fuse_post.ply.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated subset, e.g. dragon_10,horse_10,lego_10,plant_10.",
    )
    ...
    parser.add_argument("--csv-output", type=Path, default=Path("rgs_chamfer_results.csv"), help="Output CSV path.")
