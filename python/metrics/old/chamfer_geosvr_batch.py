...
DEFAULT_DATASETS: tuple[DatasetConfig, ...] = (
    DatasetConfig("horse_10", "geosvr_horse_10", "horse.ply"),
    DatasetConfig("plant_10", "geosvr_plant_10", "plant.ply"),
    DatasetConfig("lego_10", "geosvr_lego_10", "lego.ply"),
    DatasetConfig("dragon_10", "geosvr_dragon_10", "dragon.ply"),
    DatasetConfig("teapot_10", "geosvr_teapot_10", "teapot.ply"),
)
...
def resolve_reconstruction_path(output_root: Path, dataset: DatasetConfig, reconstruction_name: str) -> Path:
    reconstruction_path = output_root.expanduser() / dataset.output_folder / "mesh" / "tsdf" / reconstruction_name
    return require_existing_path(reconstruction_path, f"GeoSVR reconstruction for dataset '{dataset.name}'")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Chamfer evaluation over multiple GeoSVR datasets.")

    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/home/magnus/phd/GeoSVR/output"),
        help="Root folder containing GeoSVR output folders.",
    )
    ...
    parser.add_argument(
        ...
        help="GeoSVR mesh filename inside each dataset's mesh/tsdf folder.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated subset, e.g. horse_10,plant_10.",
    )
    ...
    parser.add_argument("--csv-output", type=Path, default=Path("geosvr_chamfer_results.csv"), help="Output CSV path.")
