...
DEFAULT_DATASETS: tuple[DatasetConfig, ...] = (
    DatasetConfig("dragon_10", "dragon_10_neus2", "dragon.ply"),
    DatasetConfig("horse_10", "horse_10_neus2", "horse.ply"),
    DatasetConfig("lego_10", "lego_10_neus2", "lego.ply"),
    DatasetConfig("plant_10", "plant_10_neus2", "plant.ply"),
    DatasetConfig("teapot_10", "teapot_10_neus2", "teapot.ply"),
)
...
def latest_obj_path(mesh_root: Path, dataset: DatasetConfig) -> Path:
    mesh_root = require_existing_path(mesh_root, f"NeuS2 mesh folder for dataset '{dataset.name}'")
    obj_paths = sorted(
        mesh_root.glob("*.obj"),
        key=lambda path: (numeric_stem(path), path.stat().st_mtime),
        reverse=True,
    )

    if not obj_paths:
        raise FileNotFoundError(
            f"Could not find latest NeuS2 reconstruction for dataset '{dataset.name}'. "
            f"Expected files matching: {mesh_root}/*.obj"
        )

    return obj_paths[0].expanduser().resolve()
...
def resolve_reconstruction_path(output_root: Path, dataset: DatasetConfig, iteration: int | None) -> Path:
    mesh_root = output_root.expanduser() / dataset.output_folder / "mesh"

    if iteration is None:
        return latest_obj_path(mesh_root, dataset)

    reconstruction_path = mesh_root / f"{iteration}.obj"
    return require_existing_path(
        reconstruction_path,
        f"NeuS2 reconstruction for dataset '{dataset.name}' at iteration {iteration}",
    )
...
def evaluate_dataset(args: argparse.Namespace, dataset: DatasetConfig, device) -> dict:
    ...
    return {
        ...
        "iteration": iteration if iteration is not None else "latest",
        "cd": metrics["cd"],
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Chamfer evaluation over multiple NeuS2 datasets.")

    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/home/magnus/projects/NeuS2/output"),
        help="Root folder containing NeuS2 output folders.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=15_000,
        help="NeuS2 OBJ iteration to evaluate, e.g. 15000. Use -1 to select latest mesh/*.obj.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated subset, e.g. dragon_10,lego_10.",
    )
    ...
    parser.add_argument("--csv-output", type=Path, default=Path("neus2_chamfer_results.csv"), help="Output CSV path.")
